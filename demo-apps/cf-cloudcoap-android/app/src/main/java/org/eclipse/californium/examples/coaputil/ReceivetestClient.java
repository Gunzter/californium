...
public class ReceivetestClient {
    ...
    public static List<String> parse(String errors) {
        List<String> rids = new ArrayList<>();
        String[] splitErrors = errors.split(SEP);
     ...
     * text.
     */
    public static String processJSON(String payload, List<String> errors, boolean verbose, List<Long> droppedResponseTimes) {
        ...
        try {
            ...
            if (verbose && element.isJsonArray()) {
                ...
                try {
                    ...
                    for (JsonElement item : element.getAsJsonArray()) {
                        ...
                        if (object.has("rid")) {
                            ...
                            if (rid.startsWith(REQUEST_ID_PREFIX)) {
                                ...
                                if (hit) {
                                    droppedResponseTimes.add(requestTime);
                                }
                    ...
                    if (!droppedResponseTimes.isEmpty()) {
                        statistic.append(" * lost responses!");
                        statistic.append(StringUtil.lineSeparator());
                    }
                    if (first != null) {
                        while (!errors.isEmpty()) {
                            if (errors.get(0).compareTo(first) >= 0) {
                                break;
                            }
                            errors.remove(0);
                        }
