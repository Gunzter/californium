...
public class SettingsFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        long minInterval = JobManager.supportedMinimumInterval(TimeUnit.MINUTES);
        if (0 < minInterval) {
            initListPreference(preference, R.string.interval_time_min, minInterval);
        } else {
    ...
    private void initListPreference(ListPreference preference, int textId, long minValue) {
        CharSequence[] values = preference.getEntryValues();
        CharSequence[] entries = preference.getEntries();
        for (int index = 1; index < values.length; ++index) {
            ...
            int left = values.length - index;
            System.arraycopy(values, index, values, 1, left);
            values = Arrays.copyOf(values, left + 1);
            break;
        }
